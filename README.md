...
**MBTI로 친구와의 관계를 시각화하고, 조합별 상세 궁합을 읽는 모바일 중심 웹서비스**
...
- 프로젝트 형태: AI 보조 개발을 활용한 개인 웹서비스 프로젝트
...
성공 주소로 돌아왔다는 이유만으로 본문을 공개하지 않습니다. 서버가 저장한 주문의 990원과 요청 금액을 비교하고, 토스 응답의 주문번호·결제 식별자·통화·금액·잔액·완료 상태를 검증합니다. 결제 비밀키는 서버 설정에만 보관합니다.
...
- 분석·결제 관련 테스트 9개 통과 및 타입 검사·배포 빌드 완료.
...
프로젝트 기획자는 서비스 주제와 기능 범위를 정하고, 브랜드·화면 문구·관계 선택·분석 분량·990원 가격을 결정했습니다. 실제 화면에 피드백을 남기고 테스트 계정 설정과 테스트 결제 확인에 참여했습니다.

Codex를 활용해 코드 작성, 유형별 원고 초안, 테스트 작성·실행, 오류 수정과 배포 작업을 진행했습니다. 따라서 이 프로젝트는 모든 코드를 수작업으로 작성한 사례가 아니라, **요구사항을 구체화하고 AI가 구현한 결과를 단계별로 확인하며 서비스를 완성해 가는 협업 사례**입니다.
